            ...
            if(!empty($data['string']))
            {
                ...
                {  
                    if(!empty($expert_and_company))
                    {
						...
						/*$append_string_in_sql = '';
						foreach ($expert_and_company as $key => $value) 
                        {  
						
							foreach($value_q['search_col_name'] as $key => $v){
								if($key > 0){
									$append_string_in_sql .= ' OR '.$v .'  LIKE "%'.$value.'%"';
								}else{
									$append_string_in_sql = $v .'  LIKE "%'.$value.'%"';
								}
							}
						}*/
